...
def magnitude_and_direction(dx, dy):
    ...
    if use_processes:
        ...
        with Pool(processes=num_of_processes, initializer=mph.magni_direc_worker, initargs=(magni_X, direc_X, dx_X, dy_X, diff, magni.shape)) as pool:
...
def sobel_threaded_manager(gau_img):
    ...
    with Pool(processes=2, initializer=mph.sobel_worker, initargs=(gau_img_X, gau_img.shape)) as pool:
...
def convolution_2D(arr, kernel, border_type):
    ...
    pad_arr = cv2.copyMakeBorder(arr, pad_amount, pad_amount, pad_amount, pad_amount, border_type)
    ...
    if rgb:
        if use_processes:
            ...
            # Using processes
            pad_arr_X = RawArray('d', pad_arr.shape[0] * pad_arr.shape[1] * pad_arr.shape[2])
            pad_arr_X_np = np.frombuffer(pad_arr_X).reshape(pad_arr.shape)
            np.copyto(pad_arr_X_np, pad_arr)

            pad_arr_out_X = RawArray('d', pad_arr_out.shape[0] * pad_arr_out.shape[1] * pad_arr_out.shape[2])
            ...
            with Pool(processes=3, initializer=mph.convolution_worker, initargs=(pad_arr_X, pad_arr_out_X, pad_arr.shape, kernel_X, kernel.shape, index)) as pool:
            ...
            for d in range(0, s):
                ...
                for x_index in range(index, x - index):
                    for y_index in range(index, y - index):
                        pad_arr_out[x_index][y_index][d] = np.sum(np.multiply(kernel, pad_arr[x_index - index:x_index + index + 1, y_index - index:y_index + index + 1,d]))
    else:
        for x_index in range(index, x - index):
            for y_index in range(index, y - index):
                pad_arr_out[x_index][y_index] = np.sum(np.multiply(kernel, pad_arr[x_index - index:x_index + index + 1, y_index - index:y_index + index + 1]))
...
def gaussian_kernel_2D(ksize, sigma):
    ...
    for x in range(ksize):
        for y in range(ksize):
            ...
            gau_kernel[x][y] = (1. / (2. * math.pi * sigma ** 2)) * pow(math.e, exponent)
...
def non_maximum_suppression(arr_mag, arr_dir):
    ...
    if use_processes:
        # Using processes
        arr_local_maxima_X = RawArray('d', arr_local_maxima.shape[0] * arr_local_maxima.shape[1])
        arr_local_maxima_X_np = np.frombuffer(arr_local_maxima_X).reshape(arr_local_maxima.shape)
        ...
        with Pool(processes=num_of_processes, initializer=mph.maxima_worker,initargs=(arr_local_maxima_X, arr_mag_X, arr_dir_X, diff, arr_mag.shape)) as pool:
...
def hysteresis_thresholding(arr, low_ratio, high_ratio):
    ...
    min_th = mean * (low_ratio/255.) * (high_ratio/low_ratio)
    max_th = mean * (high_ratio/255.) * (high_ratio/low_ratio)
    ...
    if use_processes:
        ...
        with Pool(processes=num_of_processes, initializer=mph.hysterisis_worker, initargs=(arr_X, edges_X, edges.shape, min_th, max_th, diff)) as pool:
        ...
        while changes > 0:
            ...
            for i in range(1, edges.shape[0] - 1):
                for j in range(1, edges.shape[1] - 1):
                    if (arr[i][j] >= min_th) and (arr[i][j] < max_th) and (edges[i][j] == 0):
