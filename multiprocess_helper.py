...
def convolution_RGB_threaded(dimension):
    ...
    pad_arr = np.frombuffer(convolution_dict['pad_arr']).reshape(convolution_dict['arr_shape'])
    pad_arr_out = np.frombuffer(convolution_dict['pad_arr_out']).reshape(convolution_dict['arr_shape'])
    kernel = np.frombuffer(convolution_dict['kernel']).reshape(convolution_dict['kernel_shape'])
    ...
    for x_index in range(index, x - index):
        for y_index in range(index, y - index):
            pad_arr_out[x_index][y_index][dimension] = np.sum(np.multiply(kernel, pad_arr[x_index - index:x_index + index + 1, y_index - index:y_index + index + 1, dimension]))
    return pad_arr_out
...
def sobel_threaded(option):
    ...
    if option == 0:
        return ['x', sobel_x(arr)]
    else:
        return ['y', sobel_y(arr)]
...
def magni_direc_threaded(i):
    magni = np.frombuffer(magni_direc_dict['magni']).reshape(magni_direc_dict['arr_shape'])
    direc = np.frombuffer(magni_direc_dict['direc']).reshape(magni_direc_dict['arr_shape'])
...
def hysteresis_threaded(i):
    ...
    """
    edges = np.frombuffer(hysterisis_dict['edges']).reshape(hysterisis_dict['arr_shape'])
